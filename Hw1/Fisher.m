...
function [trainError, SD_train, testError,SD_test] = FisherLD ( filename, num_crossval )
    ...
    for noOfSampleGroup = 1:num_crossval
        ...
        cov_D = zeros(size((X_train(1) - means)' * (X_train(1) - means)));
        for i = 1 : noOfClasses
            cov_D = cov_D + (X_train(i) - means)' * (X_train(i) - means);
        end

        %between the class covariance
        cov_B = zeros(size((classMean{1} - means)' * (classMean{1} - means)));
        for i = 1 : noOfClasses
            cov_B = cov_B + ((classMean{i} - means)' * (classMean{i} - means)) * size(X_train_class{i}, 1);
        end
        ...
        for index = 1 : size(new_Y_train{1}', 1)
            maximumVal = -1;
            maximumInd = 1;
            for i = 1 : noOfClasses
        ...
        for index = 1 : size(new_Y_test{1}', 1)
            maximumVal = -1;
            maximumInd = 1;
            for i = 1 : noOfClasses
